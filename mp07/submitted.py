...
def minimax(board, side, flags, depth):
    ...
    if not side:  # Maximizer
        ...
        for move in moves:
            ...
            cur_move_list, cur_move_tree, minimax_value = minimax(new_board, not side, new_flags, depth - 1)
            if minimax_value > value:
                move_list = cur_move_list
                value = minimax_value
                best_move = move
            move_tree[encode(*move)] = cur_move_tree
        move_list.insert(0, best_move)
        return (move_list, move_tree, value)
    else:  # Minimizer
...
def alphabeta(board, side, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    if not side:  # Maximizer
        ...
        for move in generateMoves(board, side, flags):
            new_side, new_board, new_flags = makeMove(side, board, move[0], move[1], flags, move[2])
            cur_move_list, cur_move_tree, alphabeta_value = alphabeta(new_board, not side, new_flags, depth - 1, alpha, beta)
