...
def get_advantages(value_net: nn.Module,
    ...
    return ((advantage - torch.mean(advantage)) / (torch.std(advantage) + EPSILON))
...
def get_value_net_loss(value_net: nn.Module,
    ...
    # YOUR CODE HERE
    # raise NotImplementedError()
    loss_train = value_net(observation)
    loss = torch.nn.functional.mse_loss(loss_train, returns) 
    return loss
...
def get_vanilla_policy_gradient_loss(policy: nn.Module,
    ...
    log_prob = policy(observation)
    for b, a in enumerate(action):
        a_opt = log_prob[b,a[0]]
        loss_tensor += (a_opt * return_or_advantage[b,0])
        ...
        for _ in range(training_epochs_per_rollout):
            ...
            for batch_start in range(0, rollout_buffer.final_size, minibatch_size):
                ...
                policy_gradient_kwargs = dict(
                    ...
                    return_or_advantage=    advantages[idxr_base[batch_start:batch_stop]] if get_advantages != None else returns[idxr_base[batch_start:batch_stop]], # Fill in
